...
export function useSystemHealth({ traces, analytics, websocketStatus, isLoading, error }) {
  return useMemo(() => {
    const loadedRecords = traces.length
    const durations = traces.map(getDurationMs).filter((duration) => duration !== null)
    const statusCodes = traces.map(getStatusCode).filter((statusCode) => statusCode !== null)
    ...
    const latestTraceTime = traces.map(getTraceTime).filter(Boolean).sort((left, right) => right - left)[0] ?? null
    ...
    const noRecentTraces = !isLoading && !error && loadedRecords === 0
    ...
    return {
      ...
      summary:
        status === 'Healthy'
          ...
          : 'Operations posture is based on loaded traces, REST state, and websocket status.',
      snapshot: [
        ...
        { label: 'Loaded Records', value: loadedRecords.toLocaleString(), tone: loadedRecords > 0 ? 'success' : 'neutral' },
      ],
      signals: [
        ...
        { label: 'Trace Intake', value: loadedRecords > 0 ? 'Receiving' : 'No data', detail: `${loadedRecords.toLocaleString()} records loaded`, tone: loadedRecords > 0 ? 'success' : 'neutral' },
        { label: 'Latency', value: formatDuration(p95ResponseTime), detail: 'P95 response time', tone: elevatedResponseTimes ? 'error' : highLatencyObserved ? 'warning' : durations.length > 0 ? 'success' : 'neutral' },
